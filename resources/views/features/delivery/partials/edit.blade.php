...
<x-feature.edit>
    <x-slot:breadcrumb>
        <x-breadcrumb :list="[['label' => @__('feature/delivery.edit'), 'route' => route('change.edit', $change)]]" />
    </x-slot:breadcrumb>
    ...
    <x-slot:from>
        ...
        <form method="POST" action="{{ route('delivery.update', base64_encode($delivery->id)) }}"
            ...
            <div class="row">
                <div class="form-group text_box col-lg-6 col-md-6">
                    ...
                    <x-input id="name" class="block mt-1 w-full" type="text"
                        placeholder="{{  __('feature/delivery.placeholder.name') }}" name="name" value="{{ $delivery->name }}" required
                        autofocus />
                </div>
                <div class="form-group text_box col-12">
                    <x-textarea label="{{ __('feature/delivery.label.items') }}" name="items" value="{{ $delivery->items }}"
                ...
                <div class="form-group text_box col-lg-6 col-md-6">
                    ...
                    <x-input id="user" class="block mt-1 w-full" type="text" placeholder="{{ __('feature/delivery.placeholder.user') }}"
                        name="user" value="{{ $delivery->user }}" required autofocus />
                    <x-input-error :messages="$errors->get('user')" class="mt-2" />
                </div>
                <div class="form-group text_box col-lg-6 col-md-6">
                    <x-input-label for="password" value="{{ __('feature/delivery.label.password') }}" />
                    <x-input id="password" class="block mt-1 w-full" type="text" placeholder="{{ __('feature/delivery.placeholder.password') }}"
                ...
                <div class="form-group text_box col-lg-6 col-md-6">
                    ...
                    <x-input id="administrator" class="block mt-1 w-full" type="text"
                        placeholder="{{ __('feature/delivery.placeholder.administrator') }}" name="administrator" value="{{ $delivery->administrator }}" required
                        autofocus />
                </div>
                <div class="form-group text_box col-lg-6 col-md-6">
            ...
            <div class="d-flex align-items-center text-center">
                <button type="submit" class="btn_hover agency_banner_btn btn-bg agency_banner_btn2">@__('feature/delivery.submit')</button>
