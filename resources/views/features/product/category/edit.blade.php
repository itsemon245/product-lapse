@extends('layouts.feature.index', ['title' => 'Edit Product Category'])
@section('main')
    <div id="hx-create-package" class="sign_info">
        <h2 class=" f_600 f_size_24 t_color3 mb_40">Add package</h2>
        <form action="{{ route('product-category.update', ['product_category' => base64_encode($category->id)]) }}"
            method="POST">
            @method('PUT')
            @csrf
            <div class="row">
                <div class="form-group text_box col-lg-4 col-md-6">
                    ...
                    <x-text-input id="name" class="block mt-1 w-full" type="text" placeholder="Enter category name"
                        name="name" :value="$category->name ?? old('name')" required autofocus />
                ...
                <div class="form-group text_box col-lg-4 col-md-6">
                    ...
                    <x-text-input id="price" class="block mt-1 w-full" type="text" placeholder="Enter text color"
                        name="text_color" :value="$category->text_color ?? old('text_color')" required autofocus />
