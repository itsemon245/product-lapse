<form id="locale-switch-form" class="h-6" action="{{ route('lang.toggle') }}" method="post">
    ...
    <label class="inline-flex rtl:flex-row-reverse items-center cursor-pointer gap-2 m-0" id="locale-switch">
        ...
        <input type="checkbox" value="{{ app()->getLocale() }}"
            data-dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}" name="toggle_locale" class="sr-only peer"
    ...
    <script>
        $(document).ready(function() {
            ...
            toggle.on('click', (e) => {
                ...
                html.attr('dir', e.target.dataset.dir)
                localStorage.setItem('locale', Cipher.encrypt(e.target.value))
            })
        });
    </script>
@endpush
